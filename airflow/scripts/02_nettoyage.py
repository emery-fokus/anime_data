...
# Maintenant :
def run_nettoyage(input_file=None, index_name="anime"):
    ...
    INPUT_FILE = Path(input_file) if input_file else DATA_DIR / "anime.csv"
    OUTPUT_FILE = OUTPUT_DIR / "anime_cleaned.csv"
    ...
    # Aligner les schémas externes (JSON/XML) sur les noms attendus par le pipeline.
    alias_map = {}
    if "genre" in df.columns and "genres" not in df.columns:
        alias_map["genre"] = "genres"
    if "studio" in df.columns and "studios" not in df.columns:
        alias_map["studio"] = "studios"
    if "rating" in df.columns and "score" not in df.columns:
        alias_map["rating"] = "score"

    if alias_map:
        df = df.rename(columns=alias_map)
        for old, new in alias_map.items():
            info(f"  Alias externe : '{old}' → '{new}'")
        ok(f"{len(alias_map)} colonne(s) externe(s) harmonisée(s)")
